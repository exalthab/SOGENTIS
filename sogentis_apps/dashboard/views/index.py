# dashboard/views/dashboard_index.py
...
from dashboard.services.stats_services import (
    get_dashboard_stats,
...
def _get_validated_profile(user):
    """
    Récupère le profil économique validé de l'utilisateur (SAFE).
    """
    if not UserEconomicProfile:
        return None

    try:
        profile = UserEconomicProfile.objects.get(user=user)
    except UserEconomicProfile.DoesNotExist:
        return None

    # Champs possibles selon versions
    if hasattr(profile, "is_validated"):
        return profile if profile.is_validated else None

    if hasattr(profile, "validation_status"):
        return profile if profile.validation_status == "APPROVED" else None
...
def dashboard_index_view(request):
    """
    Vue principale du tableau de bord (PROD).
    """
    user = request.user
    profile = _get_validated_profile(user)

    if not profile:
        messages.warning(
            request,
            _("Votre compte est en attente de validation ou a été refusé.")
        )
        return redirect("accounts_users:web:profile_pending_notice")

    # Statistiques
    stats = get_dashboard_stats(user=user)
    detailed_stats = get_detailed_statistics(user=user)

    # Engagements & dons récents
    ...
    recent_activities = (
        [f"{_('Nouvel engagement')} — {e.date.strftime('%d/%m/%Y')}" for e in derniers_engagements]
        + [f"{_('Don reçu')} — {d.created_at.strftime('%d/%m/%Y')}" for d in derniers_dons]
    )
...
#     # Localisation visiteurs (exemple → peut être connecté à Google Analytics + GeoIP)
#     visitors_locations = [
#         {"country": "Sénégal", "percent": 55, "lat": 14.5, "lon": -14.5},
#         {"country": "France", "percent": 20, "lat": 46.6033, "lon": 1.8883},
...
#     # Liste TODO
#     todos = [
#         {"label": _("Contacter un nouveau donateur"), "done": True},
#         {"label": _("Publier un témoignage"), "done": False},
#     ]
...
#         "location": "Dakar",
#         "time": _("Aujourd'hui"),
#         "status": _("Soleil voilé"),
#     }

#     # Activités récentes
#     recent_activities = [
#         f"{_('Nouvel engagement')} — {e.date.strftime('%d/%m/%Y')}"
#         for e in derniers_engagements
#     ] + [
#         f"{_('Don reçu')} — {d.created_at.strftime('%d/%m/%Y')}"
...
#         # Activités récentes / météo / visiteurs / TODO
#         "recent_activities": recent_activities,
