...
from about.models import Child
...
def soci_index_view(request):
    ...
    goal = Project.objects.aggregate(goal=Sum('goal_amount'))['goal'] or Decimal('0.00')
...
def donation_view(request, child_id=None):
    child = None
    if child_id:
    ...
    if request.method == 'POST':
        ...
        if step == "1":
            ...
            return render(request, "social/donation_payment_choice.html", {"child": child})
            ...
            if not data:
                ...
                return redirect("social:donation_child", child_id=child_id) if child_id else redirect("social:donation")
    ...
    return render(request, "social/donation.html", {"child": child})
...
# 💳 Simulations d'intégrations de paiement
@login_required
