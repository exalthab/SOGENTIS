...
class WaveWebhookView(View):
    """Webhook Wave (à compléter selon doc officielle)"""
    def post(self, request, *args, **kwargs):
        # TODO: Ajouter validation officielle Wave
        return HttpResponse("Webhook Wave reçu", status=200)


@method_decorator(csrf_exempt, name='dispatch')
class PayPalWebhookView(View):
    """Webhook PayPal (à compléter selon doc officielle)"""
    def post(self, request, *args, **kwargs):
        # TODO: Ajouter validation officielle PayPal
...
def stripe_checkout_view(request, donation_id):
    ...
    #             'product_data': {'name': 'Donation SOGENTIS'},
    #         },
    #         'quantity': 1,
    #     }],
    #     mode='payment',
    #     success_url=request.build_absolute_uri(
    #         reverse('social:donation_success', kwargs={"donation_id": donation.id})
    #     ),
    ...
    return redirect(reverse("social:donation_success", kwargs={"donation_id": donation.id}))
