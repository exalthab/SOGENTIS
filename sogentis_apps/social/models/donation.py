...
class Donation(models.Model):
    ...
    # 🔹 UID reçu (stable, vérifiable)
    receipt_uid = models.CharField(
        _("UID du reçu"),
        max_length=20,
        unique=True,
        db_index=True,
        blank=True,
        null=True,
        help_text=_("Identifiant unique public du reçu (généré automatiquement)."),
    )
    ...
    target_id = models.PositiveIntegerField(_("ID brut de la cible"), null=True, blank=True)
    ...
    target_object_id = models.PositiveIntegerField(null=True, blank=True, verbose_name=_("ID de la cible"))
    ...
    def save(self, *args, **kwargs):
        # UID stable : généré une fois, puis ne change plus
        if not self.receipt_uid:
            base = f"{self.email or ''}|{self.donor_name or ''}|{timezone.now().timestamp()}"
            self.receipt_uid = hashlib.sha1(base.encode("utf-8")).hexdigest()[:12]
