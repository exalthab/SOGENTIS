...
class ProductPricing(models.Model):
    ...
    ✅ PROD: on synchronise automatiquement Product.price
    sur le prix effectif (promo si active).
    Optionnel: on synchronise Product.old_price pour afficher le prix barré.
    """
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=Q(promo_start__isnull=True)
                | Q(promo_end__isnull=True)
                | Q(promo_start__lte=models.F("promo_end")),
    ...
    # -------------------------
    # Synchronisation Product (prod-safe, sans Product.save())
    # -------------------------
    def _sync_product_prices(self):
        """
        ✅ Aligne Product.price sur le prix effectif (promo si active).
        ✅ Pour l’affichage marketplace: aligne Product.old_price si promo active,
           sinon remet old_price à NULL (optionnel mais conseillé).
        """
        try:
            ...
            # old_price seulement si la promo est réellement active
            if self.has_promo():
                updates["old_price"] = self._q2(self.base_price)
            else:
                updates["old_price"] = None

            Product.objects.filter(pk=self.product_id).update(**updates)
        except Exception:
            pass
    ...
    def save(self, *args, **kwargs):
        ...
        # ✅ IMPORTANT: ne plus écraser Product.price avec base_price
        # -> on sync sur effective_unit_price + old_price si promo
        self._sync_product_prices()
...
class BulkPrice(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # ✅ Optionnel: si tu veux, recalculer le Product.price en fonction des tiers,
        # mais en marketplace B2C on garde pricing.effective_unit_price.
        # On ne touche rien ici par défaut (prod-safe).
