...
def _safe_login_url(login_url_name: str, default: str = "/") -> str:
    try:
...
def role_required(
    user_check: Callable,
    *,
    login_url_name: str = "accounts_users:web:auth:login",
    raise_denied: bool = True,
):
    """
    - si non connecté -> redirect login + next
    - si connecté mais non autorisé -> PermissionDenied (ou redirect login si raise_denied=False)
    """
        ...
        def _wrapped(request, *args, **kwargs):
            ...
            if not (user and getattr(user, "is_authenticated", False)):
                return redirect_to_login(
                    ...
                    login_url=_safe_login_url(login_url_name),
                )
            ok = False
            try:
                ok = bool(user_check(user))
            except Exception:
                ok = False
...
# -----------------------------------------------------
# Alias pratiques (cohérents avec tes permissions)
# -----------------------------------------------------
from economic.permissions import (  # noqa: E402
    is_vendor,
    is_verified_vendor,
    is_b2b_user,
    is_b2b_manager,
)
from dashboard.permissions import is_admin  # noqa: E402

user_required = role_required(lambda u: bool(u and u.is_authenticated), raise_denied=False)
...
def any_role_required(*checks: Callable):
    def _combined(user) -> bool:
        for check in checks:
            try:
                if check(user):
                    return True
