...
def mark_issued(modeladmin, request, queryset):
    now = timezone.now()
    queryset.update(status="issued", issued_at=now)
...
class InvoiceAdmin(admin.ModelAdmin):
    ...
    search_fields = ("invoice_number", "bulk_order__company__name", "bulk_order__uuid")
    ordering = ("-created_at",)

    readonly_fields = ("uuid", "created_at", "updated_at", "issued_at", "paid_at")
    actions = [mark_issued, mark_paid, mark_cancelled]

    fieldsets = (
        (_("Commande"), {"fields": ("bulk_order",)}),
        (_("Facture"), {"fields": ("invoice_number", "status", "amount")}),
        (_("Échéances"), {"fields": ("issued_at", "due_date", "paid_at")}),
        (_("Notes"), {"fields": ("notes",)}),
        (_("Système"), {"fields": ("uuid", "created_at", "updated_at")}),
    )

    def company_col(self, obj):
        return obj.bulk_order.company

    company_col.short_description = _("Entreprise")
